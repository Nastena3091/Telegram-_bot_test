        ...
        const options = {
            ...
            axios.request(options).then(function (response) {
                ...
                genreFromServer.forEach(genreObjects => {  //перербор великого масиву з сервера, в genreObjects потрапляють внутрішні маленькі порізані масиви по 100 елементів
                    let str = genreObjects.map(genreObj => genreObj.genre).join(', '); //тут трошки складно, бо треба сліпити між собою властивість genre, map - це теж перебор масива http://xn--80adth0aefm3i.xn--j1amh/array.map
